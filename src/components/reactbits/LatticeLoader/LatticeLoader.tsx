...
import React, { useEffect, useLayoutEffect, useRef, CSSProperties } from 'react';
import './LatticeLoader.css';

const useIsomorphicLayoutEffect = typeof window !== 'undefined' ? useLayoutEffect : useEffect;

const PATTERNS: Record<string, Record<number, { cells: (number | null)[]; loop: number; scale: number; lit?: number }>> = {
...
export default function LatticeLoader({
  ...
  const mark: 'done' | 'error' = status === 'error' ? 'error' : 'done';
  ...
  useEffect(() => {
    ...
    if (status === 'working') {
      srRef.current.textContent = `${label}, in progress`;
    } else {
      const timeStr = showTimer ? ` ${spoken(elapsed != null ? Math.round(elapsed * 10) : dsRef.current)}` : '';
      srRef.current.textContent = `${status === 'done' ? doneLabel : errorLabel}${timeStr}`;
    }
  }, [status, label, doneLabel, errorLabel, showTimer, elapsed]);
