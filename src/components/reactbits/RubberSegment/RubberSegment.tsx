...
export default function RubberSegment({
  ...
  const measure = () => {
    ...
    const measuredW = Math.max(0, rect.width - inset * 2);
    innerW.set(measuredW);
  ...
  const land = (to: number, v: number | null, flick: boolean, withSquash: boolean) => {
    ...
    const velocityFor = (mv: { getVelocity: () => number }) => clamp(v === null ? mv.getVelocity() : v, -MAX_VELOCITY, MAX_VELOCITY);
    animate(lead, leadTo, {
  ...
  const travel = (from: number, to: number) => {
    ...
    const u = stretch / 100;
    const duration = t(DILATE);
    animate(edgeL, b.l + (Math.min(a.l, b.l) - b.l) * u, { duration, ease: EASE_OUT });
    animate(edgeR, b.r + (Math.max(a.r, b.r) - b.r) * u, { duration, ease: EASE_OUT });
