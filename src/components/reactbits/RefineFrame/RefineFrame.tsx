...
const STAGES: Record<RefineFrameStatus, StageConfig> = {
...
const DEFAULT_LABELS: Record<RefineFrameStatus, string> = {
  queued: 'Queued',
  generating: 'Analyzing Package',
  refining: 'Refining OCR & Pixels',
  complete: 'Inspection Complete',
  error: 'Scan Irregularity'
};
...
const build = (s: SimState, canvas: HTMLCanvasElement, img: HTMLImageElement, dpr: number) => {
  ...
  if (glint) {
    for (const [at, a] of [
      ...
      glint.addColorStop(at, `rgba(13, 148, 136, ${a * 0.4})`);
    }
  }
  s.glint = glint;
...
export default function RefineFrame({
  ...
  const sim = useRef<SimState>({ p: 0, raf: 0, last: 0, key: '', w: 0, h: 0, levels: [], glint: null, sent: false });
  ...
  }, [status, stageDuration, sweep]);

  const [mosaic, setMosaic] = useState(false);
  const [resolved, setResolved] = useState(false);
  const [timedOutComplete, setTimedOutComplete] = useState(false);

  useEffect(() => {
    if (status !== 'complete' || hideAfter <= 0) {
      return;
    }
    const t = setTimeout(() => setTimedOutComplete(true), hideAfter);
    return () => clearTimeout(t);
  }, [status, hideAfter]);

  const chip = showStatus && !(status === 'complete' && timedOutComplete);
  ...
  const tick = (now: number) => {
    ...
    const keep = (!c.reduce && ACTIVE.has(c.status as RefineFrameStatus)) || Math.abs(target - s.p) > 0.0005;
